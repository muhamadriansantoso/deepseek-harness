 ...
 *
 * A model pi-ai reports as non-reasoning — a catalog entry marked so, or a
 * `reasoningEfforts: false` declaration — is reported by pi-ai as supporting
 * the single level `off`. Passing that through would offer a control that
 * cannot do what it says: `off` is translated to *omitting* the reasoning
 * option, which for such a model is byte-for-byte the same request as naming
 * no effort — so a provider whose own default is to think would keep thinking
 * with `off` selected. Omitting `reasoning` entirely is the seam's way of
 * saying the capability is unavailable, which leaves the surface offering only
