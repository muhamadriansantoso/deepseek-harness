...
**Privileged methods remain loopback-pinned.** The `PRIVILEGED_METHODS` set (settings, credentials, preset management, `host.openPath`, `llm.discoverModels`) is still gated to loopback with an empty trust list. Auth does not relax this — a non-loopback authenticated user still cannot reach privileged methods. This is defense in depth.

**No change to tools, sandbox, or session model.** Tools still run on the server. `session.create` is still not in `PRIVILEGED_METHODS` (by design — the default preset already carries `bash` and filesystem tools, so pinning the switch would be a fence beside an open gate). Auth is the gate that prevents anonymous access; what an authenticated user can do is unchanged.
